...
export default function BusProfile() {
  ...
  const F = ({label,name,type='text'}:{label:string;name:keyof Omit<Bus,'id'>;type?:string}) => (
    <div>
    ...
    <div className="space-y-5">
      ...
      {(modal==='add'||modal==='edit') && (
        <Modal title={modal==='add' ? 'Ongeraho Bisi Nshya' : `Hindura — ${selected?.regNumber}`}
          ...
          <div className="grid grid-cols-2 gap-4">
            <F label="Plaque (e.g. RAA 001 A)" name="regNumber"/>
            <F label="Modeli (e.g. Yutong ZK6122)" name="model"/>
            <div>
            ...
            <F label="Umubare w'Intebe" name="capacity" type="number"/>
            <div>
            ...
            <F label="Isanaruwe rya Nyuma" name="lastService" type="date"/>
            <F label="KM Yose" name="totalKm" type="number"/>
          </div>
        </Modal>
      )}
      ...
      {modal==='delete' && selected && (
        <Modal title="Siba Bisi" onClose={() => setModal(null)}
          footer={<>
            <button onClick={() => setModal(null)} className="px-4 py-2 rounded-xl border border-gray-200 text-sm font-medium text-gray-600 hover:bg-gray-50">Reka</button>
            <button onClick={() => { deleteBus(selected.id); setModal(null) }} className="px-4 py-2 rounded-xl bg-red-600 text-white text-sm font-semibold hover:bg-red-700">Siba</button>
          </>}>
