...
export default function BusCaptain() {
  ...
  const F = ({label,name,type='text'}:{label:string;name:keyof Omit<Captain,'id'>;type?:string}) => (
    <div>
    ...
    <div className="space-y-5">
      ...
      {(modal==='add'||modal==='edit') && (
        <Modal title={modal==='add'?'Ongeraho Umushoferi':`Hindura — ${selected?.name}`}
          ...
          <div className="grid grid-cols-2 gap-4">
            <F label="Amazina Yuzuye" name="name"/>
            <F label="Telefone (+250 7…)" name="phone"/>
            <F label="Nomero y'Uruhushya" name="license"/>
            <F label="Irangira ry'Uruhushya" name="licenseExpiry" type="date"/>
            <div>
            ...
            <F label="Imyaka y'Ubuhamya" name="experience" type="number"/>
            <F label="Note (1–5)" name="rating" type="number"/>
            <F label="Italiki y'Kwitabira" name="joinDate" type="date"/>
            <div>
      ...
      {modal==='delete'&&selected && (
        <Modal title="Siba Umushoferi" onClose={() => setModal(null)}
          footer={<>
            <button onClick={() => setModal(null)} className="px-4 py-2 rounded-xl border border-gray-200 text-sm font-medium text-gray-600 hover:bg-gray-50">Reka</button>
            <button onClick={() => { deleteCaptain(selected.id); setModal(null) }} className="px-4 py-2 rounded-xl bg-red-600 text-white text-sm font-semibold hover:bg-red-700">Siba</button>
          </>}>
